...
Node.prototype.move = function(newParent, toDelete, kickout){
	var that, i;
	toDelete = toDelete === undefined ? [] : toDelete;
	kickout = kickout === undefined ? true : kickout; //kickout und toDelete sorgt dafür, dass es mit allen kindern funktioniert, hoffentlich ...
	toDelete.push(this.treeId);
	that = new Node(this.content, newParent.tree);
	that.treeId = newParent.tree.counter.id();
	that.parent = newParent.treeId;
	newParent.children.push(that.treeId);
	that.tree[that.treeId] = that;
	for (i = 0; i < this.children.length; i += 1){
		kickout = false;
		if (i === this.children.length-1 && this.tree[this.children[i]].children.length === 0){
			kickout = true;
		}
		this.tree[this.children[i]].move(that, toDelete, kickout);
	}
	if (kickout){
		for (i = 0; i < toDelete.length; i += 1){
			if (this.tree[toDelete[i]] !== undefined){
				this.tree[toDelete[i]].remove();
			}
		}
	}
	return that;
...
Node.prototype.moveAfter = function(afterThat, toDelete, kickout){
	var that, i, newParent;
	...
	newParent.children.splice(index + 1, 0, that.treeId);
	that.tree[that.treeId] = that;
	for (i = 0; i < this.children.length; i += 1){
		kickout = false;
		if (i === this.children.length-1 && this.tree[this.children[i]].children.length === 0){
...
Node.prototype.moveBefore = function(beforeThat, toDelete, kickout){
	var that, i, newParent;
	newParent = beforeThat.getParent();
	toDelete = toDelete === undefined ? [] : toDelete;
	kickout = kickout === undefined ? true : kickout; //kickout und toDelete sorgt dafür, dass es mit allen kindern funktioniert, hoffentlich ...
	toDelete.push(this.treeId);
	that = new Node(this.content, newParent.tree);
	...
	newParent.children.splice(index, 0, that.treeId);
	that.tree[that.treeId] = that;
	for (i = 0; i < this.children.length; i += 1){
		kickout = false;
		if (i === this.children.length-1 && this.tree[this.children[i]].children.length === 0){
...
Tree.deserialize = function(seed, counter){ //ACHTUNG: Zur Deserialisierung von tree, schreibt man also:
	...
	counter.counter = seed.counterState-1; //wegen Root im Konstruktor
